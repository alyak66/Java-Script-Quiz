...
var questionArray = [
    ...
    {
        text: "What is the correct way to us an array in JavaScript?",
        choice1: 'var colors = "yellow", "black", "red"',
        choice2: 'var colors = ["orange", "purple", "green"] (*)',
        choice3: 'var colors = (1:"red", 2:"yellow", 3:"black")',
        choice4: 'var colors = 1 = ("purple"), 2 = ("green"), 3 = ("orange")',
        answer: "2"
    },
    {
    ...
    },
    {
        ...
        choice1: "'Comment in js",
        choice2: "<!-- Comment in js -->",
        choice3: "rem Comment in js",
        choice4: "//Comment in js",
        answer: "4"
    },
    {
...
//creates the view high scores link and timer in the header section
...
//creates the quiz title. The element the title is written to in the HTML will also server as where the questions will go
...
//creates the quiz instructions. These will be gone once the start button is clicked.
...
var initializeQuiz = function () {
    ...
    //create the feedback elements that will display if an answer is correct or wrong after it is selected when the elements are initially created, there is no text given to them, so they remain invisible until text is added later
...
//this function is called when an answer choice is clicked on. it determines which choice was clicked on, displays whether or not the selected answer was correct or wrong, deducts the time penalty if wrong, replaces the question and choices with the next question, and if the quiz is over it will determine if the submit button is clicked when the player enters their initials.
var nextQuestion = function(event) {
    ...
    //checks if we have reached the end of the quiz and that we have a valid answer I needed to add the +1 to account for the array index starting at 0 need to make sure answer is not null so the question won't advance if there is no valid answer from the previous question, i.e. on tbe first time this if statement is encountered.
    ...
    //if the event passed to this function was due to the Submit button being clicked when the player submits initials, call the submitInitials function and pass on the event to that function. I initially tried to capture the click with the submit event handler below, but it would only capture on the enter key in the text field. I suspect this is because of the alreayd exiting event listener for click on the questionWrapperEl
    if (targetEl.id === "initials-button") {
        submitInitials(event);
    }
}

//this function sets the countdown timer. timeInterval is declared as a global because the interval is cleared in the quizOver function. I found that clearing the interval in this function could result in istances where the final question was answered and then the quiz wouldn't end for another second which could result in a different number being displayed in the timer and in the final score.
...
//this function is called once the quiz is over. it stops the timer from running, removes the answer choices, displays the final score, displays the form proompting the user to enter their initials
...
var submitInitials = function (event) {
    ...
    //add the new initials/score to the end of the savedScores array and then save them back to localStorage actually sorting the scores is handled by the scores.js script
...
var clearFeedback = function (event) {
    var targetEl = event.target;
    var answer = targetEl.getAttribute("choice-number");
    //if answer exists, i.e. if there is a correct or wrong feedback message currently displayed, replace the text with "" to effectively remove the string
    if (answer) {
        var feedbackMsgEl = document.querySelector("#feedback-message");
        feedbackMsgEl.textContent = "";
    }
